...
class userTest extends TestCase
{
    ...
    public function test_userChange()
    {
        ...
        $update_url = route('change.update', $user->id);

        $response = $this->PATCH($update_url, $itemdata);
        ...
        $this->assertDatabaseHas('users', ['name' => '編集成功']);
    }
   ...
   // 認証ユーザーにして、作成ページに行ける事
    $user = User::factory()->create(['role' === 11]);
    ...
    $response = $this->post(route('message.store', $posttext));
...
//    // 保存したitemがデータベースに存在するか確認。
   ...
   // 先ほど投稿したitemのtitleと一致するものが表示されているか
    ...
    public function test_comment()
    {
        ...
        $response->assertRedirect(route('detail.show', $message->id)); 
        ...
        $response->assertSeeText($itemdata['comment']);
    }
