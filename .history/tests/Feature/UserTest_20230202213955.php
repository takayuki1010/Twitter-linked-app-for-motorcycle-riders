...
class userTest extends TestCase
{
    ...
    public function test_message()
    {
        ...
        $response = $this->post($url, $messagepost);

        $response->assertSessionHasNoErrors();

        $response->assertStatus(302);

        $this->assertDatabaseHas('Message', ['Posttext' => $posttext]);

        $response = $this->get('/');

        $response->assertStatus(200);

        $response->assertSeeText('投稿');
        ...
        $response->assertSeeText($messagepost[$posttext]);
