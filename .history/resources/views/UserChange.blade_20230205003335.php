...
<section>
    ...
    <form action="{{ route('change.update', $user->id) }}" method="POST" enctype='multipart/form-data'>
    @csrf
    @method('PATCH')
        ...
        <div class="userchange">
            @if($errors->has('changename'))
                <span style="color:red; font-size: 12px;">ユーザーネームが不正です。新たなユーザーネームを10文字以内で入力してください。</span>
            @endif
            ...
            @if($errors->has('changeemail'))
                <span style="color:red; font-size: 12px;">メールアドレスが不正です。新たなメールアドレスを入力してください。</span>
            @endif
            ...
            @if($errors->has('changeimg'))
                <span style="color:red; font-size: 12px;">画像が不正です。2MBまでのjpeg、もしくはpngを投稿してください。</span>
            @endif
