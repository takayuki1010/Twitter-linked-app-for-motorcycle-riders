...
<section>
    ...
    <div class="detail-main">
        ...
        <div class="detail-createds">
            ...
            <!-- いいね表示 -->
            <div>
                @if($message->is_liked_by_auth_user())
                    <a href="{{ route('message.unlike', ['id' => $message->id]) }}" class="btn btn-success btn-sm">いいね<span class="badge">{{ $message->likes->count() }}</span></a>
                @else
                    <a href="{{ route('message.like', ['id' => $message->id]) }}" class="btn btn-secondary btn-sm">いいね<span class="badge">{{ $message->likes->count() }}</span></a>
                @endif
                {{ $message->likes->count() }}
            </div>
            <!-- いいね機能終わり -->
