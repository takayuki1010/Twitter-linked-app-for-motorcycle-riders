...
<section>
    ...
    <div class="list">
        ...
        @can('admin-higher')
            <a style="margin-bottom: 10px;" class="button" href="{{ route('admin') }}">ユーザー一覧へ</a>
        @endcan
        <br>
        ...
        <div class="top-content">
            <!-- idをdetail.phpへ渡す -->
            <!-- メッセージのidを取得し詳細画面へ -->
            <a style="text-decoration:none; color: black;" href="{{ route('detail.show', $message->id) }}">

                <!-- ユーザーのアイコン -->
                {{ dd(asset($message->message_img1)) }}
            <div class="top-img">
                    <!-- 投稿した画像の１番目のみ表示 -->
                    <img src="{{ asset($message->message_img1) }}" alt="">
                <div class="top-imgtext">
                    <!-- リレーションしたnameを表示　投稿主の名前 -->
                    <p>{{ $message->usertable->name }}</p>
                    <!-- メッセージ表示 -->
                    <div class="top-text">
                        <!-- 投稿したメッセージを表示 -->
                        <p>{{ $message->message }}</p>
