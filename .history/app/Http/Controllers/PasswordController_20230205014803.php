...
class PasswordController extends Controller
{
    ...
    */
    ...
    {
        return view('reset'); //リセットメール送信画面へ
    }
    ...
    */
    public function sendEmailResetPassword(SendEmailRequest $request)
    {
        ...
        $request->session()->put('MAIL_SENDED_SESSION_KEY', 'user_reset_password_send_email');
    ...
    */
    public function sendComplete()
    {
       // メール送信処理で保存したセッションキーに値がなければアクセスできないようにすることで不正アクセスを防ぐ
        if (session()->get('MAIL_SENDED_SESSION_KEY') !== 'user_reset_password_send_email') {
            return redirect()->route('password_reset.email.form')
    ...
    */
    public function edit(Request $request)
    {
        ...
        try {
            ...
            return redirect()->route('password_reset.email.form')
                ->with('message', __('パスワードリセットメールに添付されたURLから遷移してください。'));
        }
    ...
    */
    public function update(ResetPasswordRequest $request)
    {
        ...
        $request->session()->put('UPDATE_PASSWORD_SESSION_KEY', 'user_update_password');
    ...
    */
    public function edited()
    {
        ...
        if (session()->get('UPDATE_PASSWORD_SESSION_KEY') !== 'user_update_password') {
            return redirect()->route('password_reset.email.form')
                ->with('message', '不正なリクエストです。');
        }
