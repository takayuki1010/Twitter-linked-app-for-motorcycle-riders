...
class userController extends Controller
{
    ...
    public function regi_process(Request $request) {
        ...
        $regiimgs = $request->file('regiimg')->getClientOriginalName();

        $regiimg = $request->file('regiimg')->store('public/' . $dir, $regiimgs);

        // ファイル情報をDBに保存
        $image = new Image();
        $image->imgName = $regiimgs;
        $image->imgPath = 'storage/' . $dir . '/' . $regiimgs;
        $image->save();
        } else {
            $regiimg = null;
        }
